...
async function getCookie(cache) {
    ...
    if (!py_loginauth) {
        if (!config.pianyuan || !config.pianyuan.cookie) {
            throw new ConfigNotFoundError('pianyuan is disabled due to the lack of <a href="https://moeyy.cn/rsshub/deploy/config#route-specific-configurations">relevant config</a>');
        }
