...
async function handler(ctx) {
    if (!config.twitter || !config.twitter.consumer_key || !config.twitter.consumer_secret) {
        throw new ConfigNotFoundError('Twitter RSS is disabled due to the lack of <a href="https://moeyy.cn/rsshub/deploy/config#route-specific-configurations">relevant config</a>');
    }
