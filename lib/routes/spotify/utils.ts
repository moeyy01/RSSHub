...
async function getPublicToken() {
    if (!config.spotify || !config.spotify.clientId || !config.spotify.clientSecret) {
        throw new ConfigNotFoundError('Spotify public RSS is disabled due to the lack of <a href="https://moeyy.cn/rsshub/deploy/config#route-specific-configurations">relevant config</a>');
    }
...
async function getPrivateToken() {
    if (!config.spotify || !config.spotify.clientId || !config.spotify.clientSecret || !config.spotify.refreshToken) {
        throw new ConfigNotFoundError('Spotify private RSS is disabled due to the lack of <a href="https://moeyy.cn/rsshub/deploy/config#route-specific-configurations">relevant config</a>');
    }
